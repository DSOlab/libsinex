...
int dso::Sinex::get_solution_estimate(const char *site_codes[],
  ...
  if (goto_block("SOLUTION/ESTIMATE")) {
    fprintf(stderr, "[ERROR] Failed to find block SOLUTION/ESTIMATE in SINEX file! (traceback: %s)\n", __func__);
    return 1;
  }

  // get lines iteratively (untill end of block) ...
  char line[sinex::max_sinex_chars];
  while (m_stream.getline(line, sinex::max_sinex_chars) && std::strncmp("-SOLUTION/ESTIMATE", line, 18)) {
