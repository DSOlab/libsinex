...
int dso::sinex::SolutionEstimate::parse_line(const char *line) noexcept {
  ...
  if (!copy_untill_wsc(bypass_wsc(start, max_count, chars_skipped),
                       m_param_type, max_count - chars_skipped)) {
    fprintf(stderr,
            "[ERROR] Failed to parse parameter type from line \"%s\" "
            "(traceback: %s)\n",
            line, __func__);
    return 1;
  }
  ...
  if (!copy_untill_wsc(bypass_wsc(start, max_count, chars_skipped), m_site_code,
                       max_count - chars_skipped)) {
    fprintf(
        stderr,
        "[ERROR] Failed to parse site code from line \"%s\" (traceback: %s)\n",
        line, __func__);
    return 1;
  }
  ...
  if (!copy_untill_wsc(bypass_wsc(start, max_count, chars_skipped),
                       m_point_code, max_count - chars_skipped)) {
    fprintf(
        stderr,
        "[ERROR] Failed to parse point code from line \"%s\" (traceback: %s)\n",
        line, __func__);
    return 1;
  }
  ...
  if (!copy_untill_wsc(bypass_wsc(start, max_count, chars_skipped), m_soln_id,
                       max_count - chars_skipped)) {
    fprintf(stderr,
            "[ERROR] Failed to parse solution id from line \"%s\" (traceback: "
            "%s)\n",
            line, __func__);
    return 1;
  }
  ...
  if (!copy_untill_wsc(bypass_wsc(start, max_count, chars_skipped), m_units,
                       max_count - chars_skipped)) {
    fprintf(stderr,
            "[ERROR] Failed to parse units from line \"%s\" (traceback: %s)\n",
            line, __func__);
    return 1;
  }
  max_count = 12;
  ...
  if (start == end) {
    fprintf(
        stderr,
        "[ERROR] Failed to parse estimate from line \"%s\" (traceback: %s)\n",
        line, __func__);
    return 1;
  }
  ...
  if (start == end) {
    fprintf(stderr,
            "[ERROR] Failed to parse std. deviation from line \"%s\" "
            "(traceback: %s)\n",
            line, __func__);
    return 1;
  }
